...
class ChessMultiplayer {
    constructor() {
        ...
        this.blackTime = 600000; // 10 minutes in ms
    ...
    setupSignalingListeners() {
        this.socket.on('challenge_received', async (data) => {
            ...
            if (confirm(`${data.from} wants to play chess with you. Accept? (You will be BLACK)`)) {
    ...
    handleTimeOut(color) {
        ...
        alert(`${colorName} ran out of time! ${color === 'w' ? 'Black' : 'White'} wins!`);
        this.cleanup();
    }
    ...
    if (challengeForm) {
        challengeForm.addEventListener('submit', async (e) => {
            ...
            multiplayer.socket.once('user_status', async (data) => {
                if (!data.online) {
                    alert(`User ${opponentId} is not online`);
                    return;
                }
